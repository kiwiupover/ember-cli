...
class EmberApp {
  ...
  constructor(defaults, options) {
    ...
    this._customTransformOrder = [];
    this._customTransforms = {};
      ...
      if (this.shouldIncludeAddon(addon)) {
        if (addon.importTransforms) {
          ...
          Object.keys(transforms).forEach(transformName => {
            const transformIndex = this._customTransformOrder.indexOf(transformName);
            if (transformIndex > -1) {
              ...
              this._customTransformOrder[transformIndex] = transformName;
            } else {
              // store the transform order to reterieve later
              this._customTransformOrder.push(transformName);
            }
            ...
            this._customTransforms[transformName] = transformConfig;
          });
        }
      }
    });
  ...
  */
  _processedExternalTree() {
    if (!this._cachedExternalTree) {
      ...
      this._customTransformOrder.forEach(transformName => {
        let transformConfig = this._customTransforms[transformName];
